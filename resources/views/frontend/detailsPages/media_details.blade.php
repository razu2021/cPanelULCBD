    ...
    <section class="details-hero" style="background-image: url('{{ asset($data->thumbnail) }}');">
        ...
        <div class="container">
            <div class="hero-content">
                ...
                <div class="hero-meta">
                    <span class="meta-item">{{$data->created_at->format('D-M-Y : h:s A')}}</span>
                </div>
            </div>
        </div>
    </section>
            ...
            <div class="col-lg-8">
                <div class="details-card">
                    ...
                    <div class="thumbnail-wrapper">
                        @if($data->cover_image )
                        <img src="{{ asset($data->cover_image) }}" alt="{{ $data->title ?? 'service image' }} " loading="lazy" class="img-fluid rounded-custom">
                        @else 
                    ...
                    <div class="sidebar-box mb-4">
                        ...
                        <div class="more-services">
                            ...
                            <div class="mini-service-item">
                                @if($alldata->cover_image)
                                <img src="{{ asset($alldata->cover_image ?? '') }}" alt="{{$alldata->title ?? 'image'}} ">
                                @else 
                                <img src="{{asset('contents/website/assets/images/avater.gif')}}" alt="{{$res->title ?? ''}}" loading="lazy">
                                @endif
                                <div>
                                    <h6><a href="{{ route('details.media',[$alldata->id,$alldata->url]) }}">{{ Str::words($alldata->title,3 ?? '') }}</a></h6>
