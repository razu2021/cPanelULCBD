...
<template>
      <div class="flex flex-col space-y-1">
        <!-- Parent Button -->
        <button
          @click="toggleMenu('marketing_suit')" class="flex items-center justify-between w-full px-3 py-2 rounded-lg hover:bg-indigo-50 transition">
          ...
          <span class="text-xs text-gray-400 transition-transform duration-200"
            :class="openMenus['marketing_suit'] ? 'rotate-180 text-indigo-600' : 'rotate-0'"><ChevronDown /></span>
        </button>

        <!-- Dropdown Items -->
        <div class="ml-6 flex flex-col space-y-1 mt-1 overflow-hidden transition-all duration-300 ease-in-out"
          :style="openMenus['marketing_suit'] ? 'max-height: 500px;' : 'max-height: 0;'">
