...
const form  = useForm(
  {
    ...
    button1: props.data.button1,
    button_url: props.data.button_url,
    button_url1: props.data.button_url1,
...
const handleUpdate = () => {
  form.transform((data) => ({
    ...(data as any),
    _method: 'patch',
  })).post(route('achivement_manage.update'), {
    forceFormData: true,
       ...
       <form @submit.prevent="handleUpdate" class="space-y-5">
        ...
        <div class="grid grid-cols-12 gap-8">
            ...
            <div class="flex items-center justify-between rounded-2xl bg-gradient-to-r from-slate-900 to-slate-800 px-6 py-4 text-white shadow-lg">
                ...
                <Link :href="route('achivement_manage.all')">All Information</Link>
                </button>
            </div>
            </div>
            ...
            <div class="rounded-2xl bg-white p-6 shadow-[0_10px_30px_rgba(0,0,0,0.06)] space-y-6">
                ...
                <div class="grid grid-cols-12 gap-4">
                  <div class="col-span-12 md:col-span-6">
                    <div>
                      <label class="text-sm font-medium text-slate-600">Button Name 2</label>
                      <input type="text" placeholder="About us" v-model="form.button1"
                        class="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-indigo-500 focus:bg-white focus:ring-indigo-500">
                        <div class="text-small text-red-500" v-if="form.errors.button1">{{ form.errors.button1 }}</div>
                    </div>
                  </div>
                  <!-- col end -->
                  <div class="col-span-12 md:col-span-6">
                    <div>
                      <label class="text-sm font-medium text-slate-600">Button URL 1</label>
                      <input type="text" placeholder="Button url" v-model="form.button_url1"
                        class="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-indigo-500 focus:bg-white focus:ring-indigo-500">
                        <div class="text-small text-red-500" v-if="form.errors.button_url1">{{ form.errors.button_url1 }}</div>
                    </div>
                  </div>
                  <!-- col end -->
                </div>
