...
import { SquarePlus, Trash,SquarePen, Eye, ShieldCheck, ShieldMinus, DownloadCloud, DeleteIcon, RotateCcwIcon, Download, DatabaseBackup,File, PlusCircle } from 'lucide-vue-next';
...
const {form} = useFilter(props,'about_manage.all')

// --- use for bulk action
const bulkRoute = ref('about_manage.bulkAction')
...
<template>
      ...
      <div>
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            <DropdownMenuGroup>
              <DropdownMenuItem>
                <a :href=" route('about_manage.export_pdf') " target="_blank" class="w-full inline-flex items-center gap-2  text-sm font-medium text-yellow-600 rounded-lg">
              ...
              <!-- end -->
              <DropdownMenuItem>
                <a :href=" route('about_manage.export_pdf') " target="_blank" class="w-full inline-flex items-center gap-2  text-sm font-medium text-green-600 rounded-lg">
                  <span class="flex items-center"><Download /></span>
              ...
              <!-- end -->
              <DropdownMenuItem>
                <a :href="route('about_manage.export_pdf')" target="_blank" class="w-full inline-flex items-center gap-2  text-sm font-medium text-blue-600 rounded-lg">
                  <span class="flex items-center"><Download /></span>
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-5 bg-white rounded-2xl border border-gray-100">
        ...
        <select v-model="form.status"  class="px-3 py-2 border rounded-lg text-sm">
          ...
          <option value="0">Inactive</option>
            ...
            <Link v-show="form.search || form.status" class="text-sm bg-green-200 p-2 rounded-full text-white hover:bg-green-600 transition-all" :href="route('about_manage.all')"><RotateCcwIcon/></Link>
      </div>
  ...
  <!-- RIGHT: Action Buttons -->
      <div class="flex flex-wrap justify-end items-center gap-2 w-full lg:w-auto">
        ...
        <Link :href="route('about_manage.recycle')"
            ...
            <td class="px-4 py-3 font-medium text-gray-800">
              <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuGroup>
                      <DropdownMenuItem>
                        <Link :href="route('sectionfeature_manage.add',{id:data.id , slug:data.slug, model:'About'})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-blue-600 rounded-lg">
                      ...
                      <DropdownMenuItem>
                        <Link :href="route('about_manage.edit',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-blue-600 rounded-lg">
                      ...
                      <!-- end -->
                      <DropdownMenuItem>
                        <Link :href="route('about_manage.view',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-gray-600 rounded-lg">
                      ...
                      <!-- end -->
                      <DropdownMenuItem>
                        <button @click="confirmDelete('about_manage.softdelete',data.id)" class="w-full inline-flex items-center gap-2  text-sm font-medium text-red-300 rounded-lg">
                    ...
                    <DropdownMenuGroup>
                      ...
                      <DropdownMenuItem>
                        <Link :href="route('about_manage.active',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-green-600 rounded-lg">
                      ...
                      <!-- end -->
                      <DropdownMenuItem>
                        <Link :href="route('about_manage.deactive',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-yellow-600 rounded-lg">
                      ...
                      <!-- end -->
                       <DropdownMenuSeparator />
                        <!-- end -->
                      <DropdownMenuItem>
                        <a target="_blank" :href="route('about_manage.single_pdf_export',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-cyan-600 rounded-lg">
