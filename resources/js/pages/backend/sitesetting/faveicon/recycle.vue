...
<template>
  ...
  <AdminLayout>
      ...
      <div> <Link :href="route('faveicon.all')" class="bg-white hover:bg-gray-100  text-dark px-4 py-2 rounded shadow-lg flex items-center gap-2 w-full sm:w-auto sm:text-center"> <InfoIcon class="text-green-600"/> View Information </Link> </div>
      </div>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="data in rows" :key="data.id" class="hover:bg-blue-50 transition-colors duration-200">
            ...
            <td class="px-4 py-3 font-medium text-gray-800 text-sm">{{ data.icons ?? '' }}</td>
              ...
              <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuGroup>
                      ...
                      <!-- end -->
                      <DropdownMenuItem>
                        <button @click="confirmDelete('faveicon.delete',data.id)" class="w-full inline-flex items-center gap-2  text-sm font-medium text-red-300 rounded-lg">
