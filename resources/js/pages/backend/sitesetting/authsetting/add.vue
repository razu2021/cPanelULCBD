...
import {groups,apikeys, authsetting } from '@/authsetting'


// UseForm with remembering state
const form = useForm('auth_setting', {
  group: '',
  key: '',
  value: '',
  address: '',
  ...
  const handleSubmit = () => {
    form.post(route('auth_setting.submit'), {
      onSuccess: () => {
        form.reset()
      },
    })
        ...
        <div class="col-span-12">
          <div class="flex items-center justify-between rounded-2xl bg-black py-4 px-5 text-white shadow-lg">
            ...
            <button
              ...
              <Link :href="route('auth_setting.all')">All Information</Link>
            </button>
          </div>
        </div>
