...
const form = useForm('sytem_maintenance', {
  type: '',
  title: '',
  description: '',
  slug: '',
  ...
  const handleSubmit = () => {
    form.post(route('sytem_maintenance.submit'), {
      onSuccess: () => {
        form.reset()
      },
    })
        ...
        <div class="col-span-12">
          <div class="flex items-center justify-between rounded-2xl bg-black py-4 px-5 text-white shadow-lg">
            ...
            <button
              ...
              <Link :href="route('sytem_maintenance.all')">All Information</Link>
            </button>
          </div>
        </div>
              ...
              <div>
                <label class="text-sm font-medium text-slate-600">Select Type</label>
                  <select name="type" id="type" required  v-model="form.type" class="block w-full rounded-xl border border-slate-300 bg-white px-4 py-2.5 text-sm text-slate-700 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:outline-none transition-colors duration-200">
                    ...
                    <option value="global">Global / Down </option>
                  </select>
