...
import { FooterTheme,theme_mainfooter } from '@/footertheme/footertheme';
...
const updateTheme = (key:string) => {
  form.theme = key
  form.patch(route('manage_footer.theme_update'), {
    preserveScroll:true
  })
...
<AdminLayout>
        ...
        <div class="col-span-12 mb-5">
          <div class="flex items-center justify-between rounded-2xl bg-black py-4 px-5 text-white shadow-lg">
            ...
            <button
              ...
              <Link :href="route('manage_footer.all')">All Information</Link>
            </button>
          </div>
        </div>
        <hr>
   ...
   <div v-if="data.type === 'main_footer'" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
    <label v-for="theme in theme_mainfooter" :key="theme.key" class="border rounded-xl overflow-hidden shadow hover:shadow-xl transition relative cursor-pointer group">
