...
const form = useForm('admin_email', {
  type: '',
  title: '',
  mail: '',
  slug: '',
  ...
  const handleSubmit = () => {
    form.post(route('admin_email.submit'), {
      onSuccess: () => {
        form.reset()
      },
    })
        ...
        <div class="col-span-12">
          <div class="flex items-center justify-between rounded-2xl bg-black py-4 px-5 text-white shadow-lg">
            ...
            <button
              ...
              <Link :href="route('admin_email.all')">All Information</Link>
            </button>
          </div>
        </div>
          ...
          <div class="rounded-2xl bg-white p-6 shadow-[0_10px_30px_rgba(0,0,0,0.06)] space-y-6">
            ...
            <div class="border-b pb-4">
              ...
              <div>
                ...
                <input type="text" placeholder="Write here !" v-model="form.mail" required
                  class="mt-1 w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm focus:border-indigo-500 focus:bg-white focus:ring-indigo-500">
                  <div class="text-small text-red-500" v-if="form.errors.mail">{{ form.errors.mail }}</div>
              </div>
                <!-- end -->
