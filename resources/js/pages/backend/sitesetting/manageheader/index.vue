...
const {form} = useFilter(props,'manage_header.all')

// --- use for bulk action
const bulkRoute = ref('manage_header.bulkAction')
...
<template>
      ...
      <div>
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            <DropdownMenuGroup>
              <DropdownMenuItem>
                <a :href=" route('manage_header.export_pdf') " target="_blank" class="w-full inline-flex items-center gap-2  text-sm font-medium text-yellow-600 rounded-lg">
              ...
              <!-- end -->
              <DropdownMenuItem>
                <a :href=" route('manage_header.export_pdf') " target="_blank" class="w-full inline-flex items-center gap-2  text-sm font-medium text-green-600 rounded-lg">
                  <span class="flex items-center"><Download /></span>
              ...
              <!-- end -->
              <DropdownMenuItem>
                <a :href="route('manage_header.export_pdf')" target="_blank" class="w-full inline-flex items-center gap-2  text-sm font-medium text-blue-600 rounded-lg">
                  <span class="flex items-center"><Download /></span>
    ...
    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 p-5 bg-white rounded-2xl border border-gray-100">
        ...
        <select v-model="form.status"  class="px-3 py-2 border rounded-lg text-sm">
          ...
          <option value="0">Inactive</option>
            ...
            <Link v-show="form.search || form.status" class="text-sm bg-green-200 p-2 rounded-full text-white hover:bg-green-600 transition-all" :href="route('manage_header.all')"><RotateCcwIcon/></Link>
      </div>
  ...
  <!-- RIGHT: Action Buttons -->
      <div class="flex flex-wrap justify-end items-center gap-2 w-full lg:w-auto">
        ...
        <Link :href="route('manage_header.recycle')"
        ...
        <Button
          ...
          <Link :href="route('manage_header.add')" class="flex items-center gap-2 w-full">
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <thead class="bg-gradient-to-r from-blue-50 to-blue-100 sticky top-0 z-10">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-gray-700 font-semibold text-sm">Theme</th>
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="data in rows" :key="data.id" class="hover:bg-blue-50 transition-colors duration-200">
            ...
            <td class="px-4 py-3 font-medium text-gray-800 text-sm"> {{ data.theme ?? '' }}</td>
            ...
            <td class="px-4 py-3 font-medium text-gray-800">
              <DropdownMenu>
                  ...
                  <DropdownMenuContent>
                    <DropdownMenuGroup>
                      <DropdownMenuItem>
                       <Link :href="route('manage_header.choose_theme',{id:data.id,slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-cyan-600 rounded-lg">
                          <span class="flex items-center"><DownloadCloud /></span>
                          <span> Choose Theme </span>
                        </Link>
                      </DropdownMenuItem>
                      <DropdownMenuItem>
                        <Link :href="route('manage_header.edit',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-blue-600 rounded-lg">
                      ...
                      <!-- end -->
                      <DropdownMenuItem>
                        <Link :href="route('manage_header.view',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-gray-600 rounded-lg">
                      ...
                      <!-- end -->
                      <DropdownMenuItem>
                        <button @click="confirmDelete('manage_header.softdelete',data.id)" class="w-full inline-flex items-center gap-2  text-sm font-medium text-red-300 rounded-lg">
                    ...
                    <DropdownMenuGroup>
                      ...
                      <DropdownMenuItem>
                        <Link :href="route('manage_header.active',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-green-600 rounded-lg">
                      ...
                      <!-- end -->
                      <DropdownMenuItem>
                        <Link :href="route('manage_header.deactive',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-yellow-600 rounded-lg">
                      ...
                      <!-- end -->
                       <DropdownMenuSeparator />
                        <!-- end -->
                      <DropdownMenuItem>
                        <a target="_blank" :href="route('manage_header.single_pdf_export',{id:data.id , slug:data.slug})" class="w-full inline-flex items-center gap-2  text-sm font-medium text-cyan-600 rounded-lg">
                          <span class="flex items-center"><DownloadCloud /></span>
                          <span> Export .PDF </span>
                        </a>
                      </DropdownMenuItem>
