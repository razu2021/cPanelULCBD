...
class EmailSmtpController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($request->filled('search')){
            $query->where('mail_mailer','LIKE', '%' .$request->search .'%');
        }
        ...
        return Inertia::render('backend/sitesetting/emailsmtp/index',[
            'alldata' => $alldata ,
    ...
    public function add()
    {
        return Inertia::render('backend/sitesetting/emailsmtp/add');
    ...
    {
        $data = Emailsmtp::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/emailsmtp/show',[
            'data' => $data
        ]);
    ...
    {
        $data = Emailsmtp::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/emailsmtp/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
        ...
        $request->validate( [
                ...
                'mail_mailer.required'=> 'Mailer field is Required !',
    ...
    public function softdelete($id){
        $data= Emailsmtp::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= Emailsmtp::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/sitesetting/emailsmtp/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Emailsmtp::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Emailsmtp::whereIn('id',$ids)->get();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Emailsmtp::where('id',$id)->where('slug',$slug)->firstOrFail();
