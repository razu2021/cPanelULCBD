...
use App\Exports\sitesetting\CookieExport;
...
class CookiePolicyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/sitesetting/cookiepolicy/index',[
            'alldata' => $alldata ,
    ...
    public function add()
    {
        ...
        return Inertia::render('backend/sitesetting/cookiepolicy/add');
    ...
    {
        $data = Cookiepolicy::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/cookiepolicy/show',[
            'data' => $data
        ]);
    ...
    {
        $data = Cookiepolicy::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/cookiepolicy/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
        ...
        if ($request->hasFile('thumbnail')) {
            ...
            $insert = Cookiepolicy::where('id', $id)
                        ->where('id', $id)->update([
                            'thumbnail' => $upload,
                        ]);
        }
    ...
    public function update(Request $request){
        ...
        // ----- insert record into database 
        $update = Cookiepolicy::where('id',$id)->where('slug',$slug)->update([
            'heading'=>$request->heading,
            'title'=>$request->title,
            'description'=>$request->description,
            'public_status'=>$request->public_status ?? 0,
        ...
        if ($request->hasFile('thumbnail')) {
            ...
            $exixtimage = Cookiepolicy::where('id', $id)->first();
        ...
        if($update){
            ...
            return redirect()->route('cookie_policy.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = Cookiepolicy::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status' => 1,
        ]);
    ...
    public function deactive($id,$slug){
        ...
        $active = Cookiepolicy::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status' => 0,
        ]);
    ...
    public function softdelete($id){
        $data= Cookiepolicy::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= Cookiepolicy::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/sitesetting/cookiepolicy/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Cookiepolicy::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Cookiepolicy::whereIn('id',$ids)->where('public_status',0)->update([
                'public_status'=>1,
            ]);
        ...
        if($action === 'InActive'){
            $categorys = Cookiepolicy::whereIn('id',$ids)->where('public_status',1)->update([
                'public_status'=>0,
            ]);
        }
        // ---------- Multiple Items Heard Delete code start here ----------
        if($action === 'Heard_Delete'){
            $categorys = Cookiepolicy::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Cookiepolicy::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new CookieExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new CookieExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Cookiepolicy::where('id',$id)->where('slug',$slug)->firstOrFail();
    ...
    public function export_excel(){
        return Excel::download(new CookieExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new CookieExport, now().'.csv');
    }
