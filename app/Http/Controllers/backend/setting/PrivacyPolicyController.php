...
use App\Exports\sitesetting\PrivacyPolicyExport;
...
class PrivacyPolicyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/sitesetting/privacypolicy/index',[
            'alldata' => $alldata ,
    ...
    public function add()
    {
        ...
        return Inertia::render('backend/sitesetting/privacypolicy/add');
    ...
    {
        $data = PrivacyPolicy::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/privacypolicy/show',[
            'data' => $data
        ]);
    ...
    {
        $data = PrivacyPolicy::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/privacypolicy/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
        ...
        if ($request->hasFile('thumbnail')) {
            ...
            $insert = PrivacyPolicy::where('id', $id)
                        ->where('id', $id)->update([
                            'thumbnail' => $upload,
                        ]);
        }
    ...
    public function update(Request $request){
        ...
        // ----- insert record into database 
        $update = PrivacyPolicy::where('id',$id)->where('slug',$slug)->update([
            'heading'=>$request->heading,
            'title'=>$request->title,
            'description'=>$request->description,
            'public_status'=>$request->public_status ?? 0,
        ...
        if ($request->hasFile('thumbnail')) {
            ...
            $exixtimage = PrivacyPolicy::where('id', $id)->first();
    ...
    public function active($id,$slug){
        $active = PrivacyPolicy::where('id',$id)->where('slug',$slug)->where('public_status',0)->update([
            'public_status' => 1,
        ]);
    ...
    public function deactive($id,$slug){
        ...
        $active = PrivacyPolicy::where('id',$id)->where('slug',$slug)->where('public_status',1)->update([
            'public_status' => 0,
        ]);
    ...
    public function softdelete($id){
        $data= PrivacyPolicy::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= PrivacyPolicy::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/sitesetting/privacypolicy/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = PrivacyPolicy::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = PrivacyPolicy::whereIn('id',$ids)->where('public_status',0)->update([
                'public_status'=>1,
            ]);
        ...
        if($action === 'InActive'){
            $categorys = PrivacyPolicy::whereIn('id',$ids)->where('public_status',1)->update([
                'public_status'=>0,
            ]);
        }
        // ---------- Multiple Items Heard Delete code start here ----------
        if($action === 'Heard_Delete'){
            $categorys = PrivacyPolicy::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = PrivacyPolicy::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new PrivacyPolicyExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new PrivacyPolicyExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = PrivacyPolicy::where('id',$id)->where('slug',$slug)->firstOrFail();
    ...
    public function export_excel(){
        return Excel::download(new PrivacyPolicyExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new PrivacyPolicyExport, now().'.csv');
    }
