...
use App\Exports\sitesetting\ApiExport;
...
class ApiSettingController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/sitesetting/apisetting/index',[
            'alldata' => $alldata ,
    ...
    public function add()
    {
        return Inertia::render('backend/sitesetting/apisetting/add');
    ...
    {
        $data = Apisetting::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/apisetting/show',[
            'data' => $data
        ]);
    ...
    {
        $data = Apisetting::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/apisetting/edit',[
            'data' => $data
        ]);
    ...
    public function update(Request $request){
        ...
        $id = $request->id;
        // ----- insert record into database 
        $update = Apisetting::where('id',$id)->where('slug',$slug)->firstOrFail();
        if($update){
            $update->update([
            'group'=>$request->group,
            'key'=>$request->key,
            ...
            return redirect()->route('api_setting.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = Apisetting::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        $active = Apisetting::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function softdelete($id){
        $data= Apisetting::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= Apisetting::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/sitesetting/apisetting/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Apisetting::whereIn('id',$ids)->get();
        ...
        if($action === 'active'){
            $categorys = Apisetting::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Apisetting::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Apisetting::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Apisetting::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new ApiExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new ApiExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Apisetting::where('id',$id)->where('slug',$slug)->firstOrFail();
    ...
    public function export_excel(){
        return Excel::download(new ApiExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new ApiExport, now().'.csv');
    }
