...
use App\Exports\sitesetting\PreloaderExport;
...
class PreloaderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/sitesetting/preloader/index',[
            'alldata' => $alldata ,
    ...
    public function add()
    {
        ...
        return Inertia::render('backend/sitesetting/preloader/add');
    ...
    {
        $data = Preloader::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/preloader/show',[
            'data' => $data
        ]);
    ...
    {
        $data = Preloader::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/preloader/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
        ...
        if ($request->hasFile('thumbnail')) {
            ...
            $upload = (new ImageUploadService($request->file('thumbnail')))
                        ->setPath('uploads/website/')->setResize(500, 500)->setOldImage($oldimage ?? '')->upload();
            // ------  save image in database 
            $insert = Preloader::where('id', $id)
                        ->where('id', $id)->update([
                            'thumbnail' => $upload,
                        ]);
        }
    ...
    public function update(Request $request){
        ...
        if ($request->hasFile('thumbnail')) {
            ...
            $exixtimage = Preloader::where('id', $id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/sitesetting/preloader/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Preloader::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Preloader::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new PreloaderExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new PreloaderExport($ids), now().'.csv');
        }
        return back();
    ...
    public function export_excel(){
        return Excel::download(new PreloaderExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new PreloaderExport, now().'.csv');
    }
