...
class AuthSettingController extends Controller
{
    ...
    public function update(Request $request){
        ...
        $id = $request->id;
        // ----- insert record into database 
        $update = Authsetting::where('id',$id)->where('slug',$slug)->firstOrFail();
        if($update){
           $update->update([
            'group'=>$request->group,
            'key'=>$request->key,
    ...
    public function active($id,$slug){
        $active = Authsetting::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        $active = Authsetting::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Authsetting::whereIn('id',$ids)->get();
        ...
        if($action === 'active'){
            $categorys = Authsetting::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Authsetting::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
                $items->update(['public_status'=>1,]);
            }
        }
