...
use App\Exports\sitesetting\ManageHeaderExport;
...
class ManageHeaderController extends Controller
{
    /**
     * ======== index page function 
     */
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/sitesetting/manageheader/index',[
            'alldata' => $alldata ,
    ...
    public function add()
    {
        return Inertia::render('backend/sitesetting/manageheader/add');
    ...
    {
        $data = Manageheader::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/manageheader/show',[
            'data' => $data
        ]);
    ...
    {
        $data = Manageheader::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/manageheader/edit',[
            'data' => $data
        ]);
    ...
    public function ThemeChoose($id,$slug){
        $data =  Manageheader::where('id',$id)->where('slug',$slug)->firstOrFail();
         return Inertia::render('backend/sitesetting/manageheader/headertheme',[
            'data' => $data
        ]);
    }
    ...
    public function themeUpdate(Request $request){
        $id = $request->id ;
        $slug = $request->slug;
        $themeupdate = Manageheader::where('id',$id)->where('slug',$slug)->firstOrFail();
        //---------------------- if insert ------
        if($themeupdate){
    ...
    public function softdelete($id){
        $data= Manageheader::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= Manageheader::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/sitesetting/manageheader/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Manageheader::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Manageheader::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new ManageHeaderExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new ManageHeaderExport($ids), now().'.csv');
        }
        return back();
    ...
    public function export_excel(){
        return Excel::download(new ManageHeaderExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new ManageHeaderExport, now().'.csv');
    }
