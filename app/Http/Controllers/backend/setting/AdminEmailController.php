...
use App\Exports\sitesetting\AdminEmailExport;
...
class AdminEmailController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/sitesetting/adminemail/index',[
            'alldata' => $alldata ,
    ...
    public function add()
    {
        return Inertia::render('backend/sitesetting/adminemail/add');
    ...
    {
        $data = AdminEmail::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/adminemail/show',[
            'data' => $data
        ]);
    ...
    {
        $data = AdminEmail::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/sitesetting/adminemail/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
        ...
        $request->validate( [
                'type' => ['required'],
                'title' => ['required'],
                'mail' => ['required', 'string', 'max:255','email',Rule::unique('admin_emails','mail')],

            ],[
                'type.required'=> 'Email Type field is Required !',
                'title.required'=> 'Email Title field is Required !',
    ...
    public function update(Request $request){
        ...
        $id = $request->id;
        // ----- insert record into database 
        $update = AdminEmail::where('id',$id)->where('slug',$slug)->firstOrFail();
        ...
        if($update){
            ...
            AdminEmail::normalizeOrder();
            flash()->success('Information Updated successfully!');
            return redirect()->route('admin_email.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = AdminEmail::where('id',$id)->where('slug',$slug)->where('public_status',0)->fristOrFail();
    ...
    public function deactive($id,$slug){
        $active = AdminEmail::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function softdelete($id){
        $data= AdminEmail::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= AdminEmail::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/sitesetting/adminemail/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = AdminEmail::whereIn('id',$ids)->get();
        ...
        if($action === 'active'){
            $categorys = AdminEmail::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = AdminEmail::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = AdminEmail::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = AdminEmail::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new AdminEmailExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new AdminEmailExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = AdminEmail::where('id',$id)->where('slug',$slug)->firstOrFail();
    ...
    public function export_excel(){
        return Excel::download(new AdminEmailExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new AdminEmailExport, now().'.csv');
    }
