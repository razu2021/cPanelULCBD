...
use App\Exports\userAppoinmentExport;
...
class userAppointmentManage extends Controller
{
    /**
     * ======== index page function 
     */
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/usermanage/userappointment/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/usermanage/userappointment/add',['section_id'=>$section]);
    ...
    {
        $data = getappointment::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/usermanage/userappointment/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
       ...
       $url = Str::slug($request->case_title) . '-' . rand(1000, 9999);
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $insert = getappointment::where('id', $id)
                        ->where('id', $id)->update([
                            'cover_image' => $upload,
                        ]);
        }
    ...
    public function update(Request $request){
      ...
      $url = Str::slug($request->case_title) . '-' . rand(1000, 9999);
        ...
        // ----- insert record into database 
        $update = getappointment::where('id',$id)->where('slug',$slug)->firstOrFail();
        $update->update([
            'type'=>$request->type,
            'title'=>$request->title,
            'short_des'=>$request->short_des,
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = getappointment::where('id', $id)->first();
    ...
    public function active($id,$slug){
        $active = getappointment::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = getappointment::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function softdelete($id){
        $data= getappointment::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= getappointment::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/usermanage/userappointment/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = getappointment::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = getappointment::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = getappointment::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = getappointment::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = getappointment::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new userAppoinmentExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new userAppoinmentExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = getappointment::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/userappointment/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = getappointment::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/userappointment/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new userAppoinmentExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new userAppoinmentExport, now().'.csv');
    }
