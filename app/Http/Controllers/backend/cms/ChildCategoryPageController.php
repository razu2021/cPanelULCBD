...
use App\Exports\ChildCategoryPageExport;
...
class ChildCategoryPageController extends Controller
{
    /**
     * ======== index page function 
     */
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/childcategory/index',[
            'alldata' => $alldata ,
    ...
    public function getSubcategory($categoryId){
        ...
        return SubCategoryPage::where('category_id',$categoryId)->get();
    ...
    public function add()
    {
        ...
        return Inertia::render('backend/cms/childcategory/add',[
    ...
    {
        $data = ChildCategoryPage::with(['creator','editor','subcategory'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/childcategory/show',[
            'data' => $data
        ]);
    ...
    public function edit($id,$slug)
    {
        ...
        $data = ChildCategoryPage::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/childcategory/edit',[
            'data' => $data,
            'allcategory' => $allcategory
        ]);
    ...
    public function insert(Request $request){
        ...
        $insert = ChildCategoryPage::create([
    ...
    public function update(Request $request){
        /**--- validation code -- */
        $category = ChildCategoryPage::where('id',$request->id)->first();
        $request->validate( [
        ...
        // ----- insert record into database 
        $update = ChildCategoryPage::where('id',$id)->where('slug',$slug)->firstOrFail();
    ...
    public function active($id,$slug){
        $active = ChildCategoryPage::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = ChildCategoryPage::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function softdelete($id){
        $data= ChildCategoryPage::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= ChildCategoryPage::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/childcategory/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'active'){
            $categorys = ChildCategoryPage::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = ChildCategoryPage::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = ChildCategoryPage::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = ChildCategoryPage::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new ChildCategoryPageExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new ChildCategoryPageExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = ChildCategoryPage::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/childcategory/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = ChildCategoryPage::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/childcategory/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new ChildCategoryPageExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new ChildCategoryPageExport, now().'.csv');
    }
