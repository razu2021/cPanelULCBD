...
use App\Exports\SectionxExport;
...
class sectionxController extends Controller
{
    /**
     * ======== index page function 
     */
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/sectionx/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/sectionx/add',['section_id'=>$section]);
    ...
    {
        $data = sectionx::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/sectionx/show',[
            'data' => $data
        ]);
    ...
    {
        $data = sectionx::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/sectionx/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
         /**--- validation code -- */
        $request->validate( [
                'title' => ['required', 'string', 'max:255',Rule::unique('sectionxes','title')],
    ...
    public function update(Request $request){
              /**--- validation code -- */
        $request->validate( [
                'title' => ['required', 'string', 'max:255',Rule::unique('sectionxes','title')->ignore($request->id)],
                'short_des' => ['required', 'string'],
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = sectionx::where('id', $id)->first();
    ...
    public function softdelete($id){
        $data= sectionx::where('id',$id)->first();
        $data->delete();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/sectionx/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = sectionx::whereIn('id',$ids)->delete();
            return back();
        }
        ...
        if($action === 'Heard_Delete'){
            $categorys = sectionx::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = sectionx::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new SectionxExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new SectionxExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = sectionx::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/sectionx/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = sectionx::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/sectionx/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new SectionxExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new SectionxExport, now().'.csv');
    }
