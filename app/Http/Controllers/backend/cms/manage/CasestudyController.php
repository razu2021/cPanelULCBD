...
use App\Exports\CasestudyExport;
...
class CasestudyController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/casestudy/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/casestudy/add',['section_id'=>$section]);
    ...
    {
        $data = Casestudy::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/casestudy/show',[
            'data' => $data
        ]);
    ...
    {
        $data = Casestudy::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/casestudy/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
         /**--- validation code -- */
        $request->validate( [
                'title' => ['required', 'string', 'max:255',Rule::unique('casestudies','title')],
    ...
    public function update(Request $request){
        /**--- validation code -- */
        $request->validate( [
                'title' => ['required', 'string', 'max:255',Rule::unique('casestudies','title')->ignore($request->id)],
                'short_des' => ['required', 'string'],
            ],[
                'title.required'=> 'Title field is Required !',
                'title.unique'=> 'This Title already exists. !',
        ...
        // ----- insert record into database 
        $update = Casestudy::where('id',$id)->where('slug',$slug)->firstOrFail();
        $update->update([
            'type'=>$request->type,
            'heading'=>$request->heading,
            'sub_heading'=>$request->sub_heading,
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = Casestudy::where('id', $id)->first();
        ...
        if($update){
            ...
            return redirect()->route('casestudy_manage.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = Casestudy::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = Casestudy::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function delete($id){
        $data= Casestudy::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/casestudy/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Casestudy::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Casestudy::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Casestudy::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Casestudy::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Casestudy::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new CasestudyExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new CasestudyExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Casestudy::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/casestudy/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = Casestudy::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/casestudy/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new CasestudyExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new CasestudyExport, now().'.csv');
    }
