...
class PromotController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/promot/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/promot/add',[
            'section_id'=>$section,
            'iconlist' => $icons
        ]);
    ...
    {
        $data = Promot::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          
        return Inertia::render('backend/cms/promot/show',[
            'data' => $data,
        ]);
    ...
    public function edit($id,$slug)
    {
        $data = Promot::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          $icons = Faveicon::pluck('icons');
        return Inertia::render('backend/cms/promot/edit',[
            'data' => $data,
            'iconlist' => $icons
        ]);
    ...
    public function update(Request $request){
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = Promot::where('id', $id)->first();
        ...
        if($update){
            ...
            return redirect()->route('promot_manage.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = Promot::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = Promot::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function delete($id){
        $data= Promot::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/promot/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Promot::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Promot::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Promot::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Promot::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Promot::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new PromotExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new PromotExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Promot::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/promot/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = Promot::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/promot/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new PromotExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new PromotExport, now().'.csv');
    }
