...
use App\Exports\AcivmentExport;
...
class Achivementcontroller extends Controller
{
    ...
    */
    public function index(Request $request)
    {
        ...
        $query = Achivement::with(['pageSection.sectionable']);
        ...
        return Inertia::render('backend/cms/achivment/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/achivment/add',[
            'section_id'=>$section,
            'iconlist' => $icons
        ]);
    ...
    {
        $data = Achivement::with(['creator','editor','pageSection.sectionable'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          
        return Inertia::render('backend/cms/achivment/show',[
            'data' => $data,
        ]);
    ...
    {
        $data = Achivement::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          $icons = Faveicon::pluck('icons');
        return Inertia::render('backend/cms/achivment/edit',[
            'data' => $data,
            'iconlist' => $icons
        ]);
    ...
    public function update(Request $request){
        ...
        // ----- insert record into database 
        $update = Achivement::where('id',$id)->where('slug',$slug)->firstOrFail();
        $update->update([
            'icon'=>$request->icon,
            'count'=>$request->count,
            'title'=>$request->title,
    ...
    public function active($id,$slug){
        $active = Achivement::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = Achivement::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function softdelete($id){
        $data= Achivement::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= Achivement::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/achivment/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Achivement::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Achivement::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Achivement::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Achivement::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Achivement::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new AcivmentExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new AcivmentExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Achivement::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/achivment/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = Achivement::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/achivment/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new AcivmentExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new AcivmentExport, now().'.csv');
    }
