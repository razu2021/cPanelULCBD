...
class BlogController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/blog/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/blog/add',['section_id'=>$section]);
    ...
    {
        $data = Blog::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/blog/show',[
            'data' => $data
        ]);
    ...
    {
        $data = Blog::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/blog/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
         /**--- validation code -- */
        $request->validate( [
                'title' => ['required', 'string', 'max:255',Rule::unique('blogs','title')],
    ...
    public function update(Request $request){
         /**--- validation code -- */
        $request->validate( [
                'title' => ['required', 'string', 'max:255',Rule::unique('blogs','title')->ignore($request->id)],
                'short_des' => ['required', 'string'],
        ...
        // ----- insert record into database 
        $update = Blog::where('id',$id)->where('slug',$slug)->firstOrFail();
        $update->update([
            'type'=>$request->type,
            'heading'=>$request->heading,
            'sub_heading'=>$request->sub_heading,
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = Blog::where('id', $id)->first();
        ...
        if($update){
            ...
            return redirect()->route('blog_manage.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = Blog::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = Blog::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function delete($id){
        $data= Blog::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/blog/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Blog::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Blog::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Blog::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Blog::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Blog::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new BlogExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new BlogExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Blog::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/blog/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = Blog::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/blog/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new BlogExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new BlogExport, now().'.csv');
    }
