...
class StoryController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/story/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/story/add',['section_id'=>$section]);
    ...
    {
        $data = story::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/story/show',[
            'data' => $data
        ]);
    ...
    {
        $data = story::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/story/edit',[
            'data' => $data
        ]);
    ...
    public function update(Request $request){
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = story::where('id', $id)->first();
        ...
        if($update){
            ...
            return redirect()->route('story_manage.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = story::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = story::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function delete($id){
        $data= story::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/story/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = story::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = story::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = story::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = story::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = story::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new StoryExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new StoryExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = story::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/story/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = story::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/story/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new StoryExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new StoryExport, now().'.csv');
    }
