...
use App\Exports\CountrydestinationExport;
...
class CountrydestinationController extends Controller
{
    ...
    {
        $query = Countrydestination::query(); 
        ...
        return Inertia::render('backend/cms/country/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/country/add',[
            'section_id'=>$section,
            'iconlist' => $icons
        ]);
    ...
    {
        $data = Countrydestination::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          
        return Inertia::render('backend/cms/country/show',[
            'data' => $data,
        ]);
    ...
    public function edit($id,$slug)
    {
        $data = Countrydestination::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          $icons = Faveicon::pluck('icons');
        return Inertia::render('backend/cms/country/edit',[
            'data' => $data,
            'iconlist' => $icons
        ]);
    ...
    public function update(Request $request){
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = Countrydestination::where('id', $id)->first();
        ...
        if($update){
            ...
            return redirect()->route('countrydestination_manage.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = Countrydestination::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = Countrydestination::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function softdelete($id){
        $data= Countrydestination::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= Countrydestination::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/country/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Countrydestination::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Countrydestination::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Countrydestination::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Countrydestination::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Countrydestination::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new CountrydestinationExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new CountrydestinationExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Countrydestination::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/country/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = Countrydestination::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/country/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new CountrydestinationExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new CountrydestinationExport, now().'.csv');
    }
