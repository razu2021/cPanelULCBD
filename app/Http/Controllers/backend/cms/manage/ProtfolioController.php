...
use App\Exports\ProtfolioExport;
...
class ProtfolioController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/protfolio/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/protfolio/add',['section_id'=>$section]);
    ...
    {
        $data = Protfolio::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/protfolio/show',[
            'data' => $data
        ]);
    ...
    {
        $data = Protfolio::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/protfolio/edit',[
            'data' => $data
        ]);
    ...
    public function insert(Request $request){
         /**--- validation code -- */
        $request->validate( [
                'title' => ['required', 'string', 'max:255',Rule::unique('protfolios','title')],
    ...
    public function update(Request $request){
              /**--- validation code -- */
        $request->validate( [
                'title' => ['required', 'string', 'max:255',Rule::unique('protfolios','title')->ignore($request->id)],
                'short_des' => ['required', 'string'],
        ...
        // ----- insert record into database 
        $update = Protfolio::where('id',$id)->where('slug',$slug)->firstOrFail();
        $update->update([
            'type'=>$request->type,
            'heading'=>$request->heading,
            'sub_heading'=>$request->sub_heading,
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = Protfolio::where('id', $id)->first();
        ...
        if($update){
            ...
            return redirect()->route('protfolio_manage.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = Protfolio::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = Protfolio::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function softdelete($id){
        $data= Protfolio::where('id',$id)->first();
        $data->delete();
    ...
    public function delete($id){
        $data= Protfolio::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/protfolio/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Protfolio::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Protfolio::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Protfolio::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Protfolio::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Protfolio::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new ProtfolioExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new ProtfolioExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Protfolio::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/protfolio/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = Protfolio::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/protfolio/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new ProtfolioExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new ProtfolioExport, now().'.csv');
    }
