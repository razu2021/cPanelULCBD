...
class TestimonialController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/testimonial/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/testimonial/add',[
            'section_id'=>$section,
            'iconlist' => $icons
        ]);
    ...
    {
        $data = Testimonial::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          
        return Inertia::render('backend/cms/testimonial/show',[
            'data' => $data,
        ]);
    ...
    public function edit($id,$slug)
    {
        $data = Testimonial::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          $icons = Faveicon::pluck('icons');
        return Inertia::render('backend/cms/testimonial/edit',[
            'data' => $data,
            'iconlist' => $icons
        ]);
    ...
    public function update(Request $request){
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = Testimonial::where('id', $id)->first();
        ...
        if($update){
            ...
            return redirect()->route('testimonial_manage.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = Testimonial::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = Testimonial::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function delete($id){
        $data= Testimonial::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/testimonial/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Testimonial::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Testimonial::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Testimonial::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Testimonial::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Testimonial::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new TestimonialExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new TestimonialExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Testimonial::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/testimonial/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        ...
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/testimonial/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new TestimonialExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new TestimonialExport, now().'.csv');
    }
