...
use App\Exports\ReaserchExport;
...
class ReaserchController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/reaserch/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/reaserch/add',['section_id'=>$section]);
    ...
    {
        $data = Research::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/reaserch/show',[
            'data' => $data
        ]);
    ...
    {
        $data = Research::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
        return Inertia::render('backend/cms/reaserch/edit',[
            'data' => $data
        ]);
    ...
    public function update(Request $request){
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = Research::where('id', $id)->first();
    ...
    public function active($id,$slug){
        $active = Research::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = Research::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function delete($id){
        $data= Research::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/reaserch/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Research::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Research::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Research::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Research::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Research::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new ReaserchExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new ReaserchExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Research::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/reaserch/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = Research::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/reaserch/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new ReaserchExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new ReaserchExport, now().'.csv');
    }
