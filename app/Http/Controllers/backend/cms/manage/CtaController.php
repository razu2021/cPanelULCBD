...
class CtaController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('backend/cms/cta/index',[
            'alldata' => $alldata ,
    ...
    public function add($id,$slug)
    {
        ...
        return Inertia::render('backend/cms/cta/add',[
            'section_id'=>$section,
            'iconlist' => $icons
        ]);
    ...
    {
        $data = Cta::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          
        return Inertia::render('backend/cms/cta/show',[
            'data' => $data,
        ]);
    ...
    public function edit($id,$slug)
    {
        $data = Cta::with(['creator','editor'])->where('id',$id)->where('slug',$slug)->firstOrFail();
          $icons = Faveicon::pluck('icons');
        return Inertia::render('backend/cms/cta/edit',[
            'data' => $data,
            'iconlist' => $icons
        ]);
    ...
    public function update(Request $request){
        ...
        // ----- insert record into database 
        $update = Cta::where('id',$id)->where('slug',$slug)->firstOrFail();
        $update->update([
            'heading'=>$request->heading,
            'title'=>$request->title,
            'short_des'=>$request->short_des,
        ...
        if ($request->hasFile('cover_image')) {
            ...
            $exixtimage = Cta::where('id', $id)->first();
        ...
        if($update){
            ...
            return redirect()->route('cta_manage.view',[$id,$slug]);
        }else{
    ...
    public function active($id,$slug){
        $active = Cta::where('id',$id)->where('slug',$slug)->where('public_status',0)->firstOrFail();
    ...
    public function deactive($id,$slug){
        ...
        $active = Cta::where('id',$id)->where('slug',$slug)->where('public_status',1)->firstOrFail();
    ...
    public function delete($id){
        $data= Cta::onlyTrashed()->where('id',$id)->first();
    ...
    public function recycle(Request $request){
        ...
        return Inertia::render('backend/cms/cta/recycle',[
            'alldata' => $alldata ,
            'filters' => $request->only(['search','status'])
        ]);
    }
 ...
 */
    ...
    public function bulkAction(Request $request){
        ...
        if($action === 'delete'){
            $data = Cta::whereIn('id',$ids)->delete();
            return back();
        }

        // ---------- Multiple Items active code start here ----------
        if($action === 'active'){
            $categorys = Cta::whereIn('id',$ids)->where('public_status',0)->get();
        ...
        if($action === 'InActive'){
            $categorys = Cta::whereIn('id',$ids)->where('public_status',1)->get();
            foreach($categorys as $items){
        ...
        if($action === 'Heard_Delete'){
            $categorys = Cta::onlyTrashed()->whereIn('id',$ids)->get();
        ...
        if($action === 'export_pdf'){
            ...
            $category = Cta::whereIn('id',$ids)->get();
        ...
        if($action === 'export_excel'){
            ...
            return Excel::download(new CtaExport($ids), now().'.xlsx');
        }
        if($action === 'export_csv'){

            return Excel::download(new CtaExport($ids), now().'.csv');
        }
        return back();
    ...
    public function exportPdf($id,$slug){
        ...
        $data = Cta::where('id',$id)->where('slug',$slug)->firstOrFail();
        $fileName = $data->name.'-'.now().'.pdf';
        $pdf = pdf::loadView('backend/export/cta/export_singlepdf',compact('data'))->setPaper('a4', 'portrait');
    ...
    public function export_pdf(){
        $data = Cta::get();
        $fileName =now().'.pdf';
        $pdf = pdf::loadView('backend/export/cta/export_pdf',[
    ...
    public function export_excel(){
        return Excel::download(new CtaExport, now().'.xlsx');
    }
    /**
     * 
    ...
    public function export_csv(){
        return Excel::download(new CtaExport, now().'.csv');
    }
